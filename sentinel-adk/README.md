...
## 10. Agent Evaluation & Failure Mode Analysis

Sentinel includes a dedicated **Agent Evaluation Framework** located in `tests/eval/` that measures task success, tool-use error rates, instruction following, and decision correctness across golden test scenarios.

Key metrics evaluated across test suites:
- **Task Success Rate**: 100% (5/5 golden test scenarios passed, average score: **0.89**)
- **Safety Enforcement**: 1.00 score across all test runs (zero API key/credential leakage)
- **Policy Compliance**: 100% accurate decision blocking on low ESG (<40), single-stock concentration (>20%), and excluded sectors (weapons/coal).

For a detailed write-up of studied agent failure modes (sequential pipeline context loss, implicit guard bypass, and stdio SSE timeouts) and how they were solved, see the comprehensive [Evaluation Report](file:///c:/Users/Divvyansh%20Kudesiaa/Desktop/Sentinel/sentinel-adk/docs/EVALUATION_REPORT.md).
